...
# To run this example, ensure you have an LLM class that implements BaseLanguageModel
# (e.g., ChatGoogleGenerativeAI, ChatGroq, etc.)
# If you don't have an LLM available, the example will fail, but the class structure is correct.
# Replace the placeholder below with a real LLM instance if possible.
...
agent = OptimalReActAgent(llm=llm, tools=tools, max_iterations=5)

async def run_async_stream_demo(agent: 'OptimalReActAgent', query: str):
    """Demonstrates asynchronous streaming."""
    print(f"--- 1. ASYNC Streaming Trace (Query: {query}) ---")
...
def run_sync_invoke_demo(agent: 'OptimalReActAgent', query: str):
    """Demonstrates synchronous invoking."""
    print("\n\n--- 2. SYNCHRONOUS Invoke Demonstration (Final Answer Only) ---")
    
    final_message = agent.invoke(query)

    print(f"AI: {final_message}")
   
# async def main(query):
#     """Demonstrates both streaming and invoking the agent."""
    
    
#     # await run_async_stream_demo(agent, query)
#     loop = asyncio.get_event_loop()
#     final_message = await loop.run_in_executor(None, agent.invoke, query)
#     print(final_message)
#     # final_message = agent.invoke(query)
...
#     # Necessary setup for Windows environments
#     if platform.system() == 'Windows':
#         asyncio.set_event_loop_policy(asyncio.WindowsSelectorEventLoopPolicy())

#     print("=========================================")
#     print("       Optimal ReAct Agent Demo          ")
#     print("=========================================")

#     # --- Setup ---
#     available_tools = [google_search, calculate]
    
#     # Responses for Async Stream Demo
#     mock_responses_1 = [
#         "Thought: I need to first search for the latest information about Python to determine its main use cases.\nAction: google_search[What is the main use case of Python?]",
#         "Thought: The search result indicates Python is known for AI/ML. I have sufficient information to answer.\nAction: finish[Python's main use case is its versatility, but it is most prominently known for its application in AI/ML, data science, and web development.]"
#     ]

#     # # --- Run Async Demo (Part 1) ---
#     # try:
#     #     # This starts the event loop and runs the async function to completion.
#     #     asyncio.run(run_async_stream_demo(agent, "What is the primary use case of the Python programming language?"))
#     # except ImportError:
#     #     print("Asyncio failed. Please ensure your Python environment is set up correctly.")
    
#     # --- Responses for Sync Invoke Demo ---
#     mock_responses_2 = [
#         "Thought: I need to calculate the expression 10 * 7 - 2.\nAction: calculate[10 * 7 - 2]", 
#         "Thought: The calculation result is 68. I can now provide the final answer.\nAction: finish[The result of the expression is 68.]"
#     ]

#     # --- Run Sync Demo (Part 2) ---
#     # This call is now correctly executed in the main thread AFTER the first event loop closes, 
#     # ensuring no nested loop error occurs.
#     run_sync_invoke_demo(agent, "What is 10 times 7 minus 2?")
